...
type LogRecord struct {
	...
	Timestamp witTypes.Option[wasi_clocks_wall_clock.Datetime]
	// Time when the event was observed.
	ObservedTimestamp witTypes.Option[wasi_clocks_wall_clock.Datetime]
	// The severity text(also known as log level).
	SeverityText witTypes.Option[string]
	// The numerical value of the severity ranging from 1-24.
	SeverityNumber witTypes.Option[uint8]
	// The body of the log record.
	Body witTypes.Option[string]
	// Additional information about the specific event occurrence.
	Attributes witTypes.Option[[]wasi_otel_types.KeyValue]
	// Name that identifies the class / type of event.
	EventName witTypes.Option[string]
	// Describes the source of the log.
	Resource witTypes.Option[wasi_otel_types.Resource]
	// Describes the scope that emitted the log.
	InstrumentationScope witTypes.Option[wasi_otel_types.InstrumentationScope]
	// Request trace id.
	TraceId witTypes.Option[string]
	// Request span id.
	SpanId witTypes.Option[string]
	// W3C trace flag.
	TraceFlags witTypes.Option[wasi_otel_tracing.TraceFlags]
}
...
func OnEmit(data LogRecord) {
	...
	returnArea := uintptr(witRuntime.Allocate(pinner, (56 + 36*4), 8))
	...
	case witTypes.OptionSome:
		...
		*(*int8)(unsafe.Add(unsafe.Pointer(returnArea), (48 + 13*4))) = int8(int32(1))
		...
		result7 := witRuntime.Allocate(pinner, uintptr(length8*(4*4)), 4)
	...
	case witTypes.OptionSome:
		...
		*(*uint32)(unsafe.Add(unsafe.Pointer(returnArea), (48 + 20*4))) = uint32(uintptr(uintptr(utf810)))
		...
		result16 := witRuntime.Allocate(pinner, uintptr(length17*(4*4)), 4)
